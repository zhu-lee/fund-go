...
func (d *Document) LoadBytes(buf []byte, charset CharsetFunc) (err error) {
	return d.LoadStream(bytes.NewBuffer(buf), charset)
}
...
func (d *Document) SaveFile(path string) error {
	return ioutil.WriteFile(path, d.SaveBytes(), 0600)
}
